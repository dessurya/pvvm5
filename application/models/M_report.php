...
class M_report extends CI_Model{
	...
	public function qty($oid, $wid){
		$query = "
			SELECT 
				CASE WHEN REQUEST_QTY IS NULL THEN 0 ELSE REQUEST_QTY END AS REQUEST,
				CASE WHEN TONGKANG_QTY IS NULL THEN 0 ELSE TONGKANG_QTY END AS TONGKANG,
				CASE WHEN TRUCKING_QTY IS NULL THEN 0 ELSE TRUCKING_QTY END AS TRUCKING
	...
	public function getWasteReport(){
		...
		$query = "
			SELECT
				...
				CASE WHEN SUM(REQUEST_QTY) IS NULL THEN 0 ELSE REQUEST_QTY END AS REQUEST_QTY,
				CASE WHEN SUM(TONGKANG_QTY) IS NULL THEN 0 ELSE TONGKANG_QTY END AS TONGKANG_QTY,
				CASE WHEN SUM(TRUCKING_QTY) IS NULL THEN 0 ELSE TRUCKING_QTY END AS TRUCKING_QTY
			FROM 
